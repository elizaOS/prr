  ...
  if (structuredResult) {
    ...
    switch (structuredResult.resultCode) {
      case 'ALREADY_FIXED': {
        ...
        const firstIssueAf = unresolvedIssues[0];
        if (firstIssueAf && stateContext.state) {
          ...
          const consecutive = last === detail
            ? (state.alreadyFixedConsecutiveSameByCommentId[firstIssueAf.comment.id] ?? 0) + 1
            : 1;
      ...
      case 'CANNOT_FIX': {
        ...
        if (firstIssue0) {
          ...
          if (workdir && stateContext.state) {
            // Track CANNOT_FIX consecutive (any reason) so we dismiss after threshold (output.log audit: Vercel bot comments burned 10 iterations).
            if (structuredResult.resultCode === 'CANNOT_FIX') {
              const state = stateContext.state;
              if (!state.cannotFixConsecutiveByCommentId) state.cannotFixConsecutiveByCommentId = {};
              state.cannotFixConsecutiveByCommentId[firstIssue0.comment.id] = (state.cannotFixConsecutiveByCommentId[firstIssue0.comment.id] ?? 0) + 1;
            ...
            // Track CANNOT_FIX citing missing/placeholder file content so circuit breaker can skip after threshold.
            if (structuredResult.resultCode === 'CANNOT_FIX' && /(?:file\s+content|actual\s+file|placeholder|missing\s+from\s+the\s+prompt|not\s+(?:actual|real)\s+code)/i.test(detail)) {
              const state = stateContext.state;
              if (!state.cannotFixMissingContentCountByCommentId) state.cannotFixMissingContentCountByCommentId = {};
              state.cannotFixMissingContentCountByCommentId[firstIssue0.comment.id] = (state.cannotFixMissingContentCountByCommentId[firstIssue0.comment.id] ?? 0) + 1;
              debug('CANNOT_FIX missing content count', { commentId: firstIssue0.comment.id, count: state.cannotFixMissingContentCountByCommentId[firstIssue0.comment.id] });
            }
            // Persist other file from CANNOT_FIX detail (e.g. fix in commit.ts).
            if (structuredResult.resultCode === 'CANNOT_FIX') {
              const otherFile = parseOtherFileFromResultDetail(detail, firstIssue0.comment.path, workdir);
              if (otherFile && !isReferencePathInComment(firstIssue0.comment.body, otherFile)) {
                const state = stateContext.state;
            ...
            if (structuredResult.resultCode === 'UNCLEAR' && /test|coverage|\.test\.(ts|js)/i.test(detail) && /target|allowed|not in my|cannot add|not permitted/i.test(detail)) {
              ...
              if (testPath) {
                ...
                if (!existing.includes(testPath)) {
                  state.wrongFileAllowedPathsByCommentId[firstIssue0.comment.id] = [...existing, testPath];
            ...
            if (structuredResult.resultCode === 'UNCLEAR') {
              ...
              state.wrongLocationUnclearCountByCommentId[firstIssue0.comment.id] = (state.wrongLocationUnclearCountByCommentId[firstIssue0.comment.id] ?? 0) + 1;
              const normalizedDetail = (structuredResult.resultDetail ?? 'UNCLEAR').trim().substring(0, 120);
              ...
              const last = state.wrongLocationUnclearLastDetailByCommentId[firstIssue0.comment.id];
              const consecutive = last === normalizedDetail
                ? (state.wrongLocationUnclearConsecutiveSameByCommentId[firstIssue0.comment.id] ?? 0) + 1
                : 1;
              state.wrongLocationUnclearLastDetailByCommentId[firstIssue0.comment.id] = normalizedDetail;
              state.wrongLocationUnclearConsecutiveSameByCommentId[firstIssue0.comment.id] = consecutive;
            }
          }
        } else {
      ...
      case 'WRONG_LOCATION': {
        ...
        if (firstIssue1) {
          ...
          if (workdir && stateContext.state) {
            ...
            if (otherFile && !isReferencePathInComment(firstIssue1.comment.body, otherFile)) {
              if (!state.wrongFileAllowedPathsByCommentId) state.wrongFileAllowedPathsByCommentId = {};
            ...
            if (snippetNotVisible) {
              if (!state.widerSnippetRequestedByCommentId) state.widerSnippetRequestedByCommentId = {};
  ...
  for (let i = 0; i < unresolvedIssues.length; i++) {
    ...
    if (result && !result.exists) {
      ...
      if (result) {
        ...
        issue.verifierFeedbackHistory.push(result.explanation);
        ...
        // WHY track rejection: Same as fix-verification path — after N rejections we mark as remaining
