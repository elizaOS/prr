...
export async function executePushIteration(
  ...
  while (fixIteration < effectiveMaxFixIterations && !allFixed) {
    ...
    // Stagnation: issues that already had wrong-file edits get single-issue focus first so we don't burn batch attempts.
    // WHY cap at 2: After 3+ wrong-file attempts, skip single-issue focus to avoid endless retries (audit: wrong-file issues wasted iterations).
    const WRONG_FILE_SINGLE_ISSUE_MAX = 2;
    const wrongFileIssues = unresolvedIssues.filter((i) => {
      const n = stateContext.state?.wrongFileLessonCountByCommentId?.[i.comment.id] ?? 0;
      return n >= 1 && n <= WRONG_FILE_SINGLE_ISSUE_MAX;
    });
    ...
    if (!allFixed) {
      ...
      if (postVerif.shouldBreak) {
        exitReason = 'bail_out';
        exitDetails = `Stalemate detected: fix loop tried all strategies with ${formatNumber(unresolvedIssues.length)} issue(s) remaining`;
