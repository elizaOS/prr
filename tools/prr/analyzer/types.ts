...
export interface UnresolvedIssue {
  ...
  /**
   * Full thread of verifier rejections (oldest first). Fixer sees the
   * progression so it can address the latest feedback without repeating
   * the same attempt. Capped (e.g. 10) to avoid prompt bloat.
   */
  verifierFeedbackHistory?: string[];
  /**
  ...
  allowedPaths?: string[];
  /**
   * Resolved full path when comment.path is a basename and the diff contains
   * a file with that basename (e.g. comment on "reporting.py" → "benchmarks/bfcl/reporting.py").
