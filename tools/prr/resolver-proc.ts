...
// Constants consolidated to shared/constants.ts (canonical source of truth)
...
export async function calculateSmartWaitTime(
  ...
  if (botTimings.length > 0) {
    const { formatDuration } = await import('../../shared/logger.js');
...
export async function executeBailOut(
  ...
  // Add "remaining" comments for unresolved issues on bail-out
  // WHY: Documents for review bots and humans that these need resolution by fix, conversation, or other means.
  try {
    ...
    if (remainingIssues.length > 0) {
      ...
      const { added } = await addDismissalComments(remainingIssues, workdir, llm);
      if (added > 0) {
...
/**
 * Check for new bot reviews that arrived while working (e.g. CodeRabbit posted).
 * Called at the start of each fix iteration; new comments are injected into the fix queue.
 * No timing gate — we check every cycle so when CodeRabbit (or any bot) drops, it drops in.
 */
export async function checkForNewBotReviews(
  ...
  const { debug } = await import('../../shared/logger.js');
