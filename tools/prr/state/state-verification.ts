 ...
 *
 * Writes go through {@link transitionIssue} in state-transitions.ts so
 * commentStatuses, dismissed mutual exclusion, and apply-failure cleanup stay in sync.
 */
...
/** Optional flags for {@link markVerified} (fourth argument). */
...
export function markVerified(
  ...
  transitionIssue(ctx, commentId, {
    ...
    forceVerificationRefresh: options?.forceVerificationRefresh,
  });
}
...
export function unmarkVerified(ctx: StateContext, commentId: string): void {
  transitionIssue(ctx, commentId, { kind: 'unverified' });
}
