...
import { lessonForbidsEditingIssuePath } from './lessons-retrieve.js';
...
/**
 * Drop file-scoped lessons that forbid editing the very file they are stored under.
 * **WHY:** Same-path "do not edit this file" lessons block the fixer forever for that path (ROADMAP).
 */
export function pruneLessonsForbiddingOwnTargetPath(ctx: LessonsContext): number {
  let removed = 0;
  for (const filePath of Object.keys(ctx.store.files)) {
    const cleaned = Normalize.sanitizeFilePathHeader(filePath);
    if (!cleaned) continue;
    const list = ctx.store.files[filePath];
    const before = list.length;
    const kept = list.filter((lesson) => {
      if (lessonForbidsEditingIssuePath(lesson, cleaned)) {
        removed++;
        return false;
      }
      return true;
