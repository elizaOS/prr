 ...
 * When 1–3 files, append (file1, file2) so repeated commits with same pattern get distinct lines.
 */
export function generateCommitFirstLine(
  ...
  if (filePaths.length >= 1 && filePaths.length <= 3) {
    ...
    if (uniq.length > 0) {
      const withHint = `${base} (${uniq.join(', ')})`;
      if (withHint.length <= 72) return withHint;
    }
  }
  return base;
}
...
function extractDescription(
  ...
  // Single file + test/coverage mention → specific line for history
  if (fixedIssues.length === 1 && filePaths.length === 1) {
    const comment = fixedIssues[0]!.comment.toLowerCase();
    const base = filePaths[0]!.split('/').pop()?.replace(/\.[^.]+$/, '') || 'file';
    if (/\btest\b|\bcoverage\b/.test(comment)) return `add tests for ${base}`;
  }
