...
/** Margin (lines) when filtering a diff by line range so the verifier sees enough context. */
const DIFF_FILTER_LINE_MARGIN = 40;

/**
 * Filter a single-file unified diff to only hunks that overlap the given line range.
 * WHY: When multiple fixes target the same file, the batch verifier gets the same full diff for each;
 * filtering per issue by line avoids attributing one fix's hunks to another (prompts.log audit).
 */
export function filterUnifiedDiffByLineRange(
  diff: string,
  issueLine: number,
  margin: number = DIFF_FILTER_LINE_MARGIN
): string {
  const minLine = Math.max(1, issueLine - margin);
  const maxLine = issueLine + margin;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const diffFile = line.match(/^diff --git a\/.+? b\/(.+)/);
    if (diffFile) {
      if (hunkLines.length > 0 && keepHunk) out.push(...hunkLines);
      hunkLines = [];
      inHeader = true;
      out.push(line);
      continue;
    ...
    if (hunk) {
      ...
      const oldCount = parseInt(hunk[2] || '1', 10);
      const oldEnd = oldStart + oldCount;
      keepHunk = oldEnd >= minLine && oldStart <= maxLine;
      hunkLines = [line];
      inHeader = false;
      continue;
    }
    if (!inHeader) hunkLines.push(line);
  }
  if (hunkLines.length > 0 && keepHunk) out.push(...hunkLines);
  return out.join('\n');
}
