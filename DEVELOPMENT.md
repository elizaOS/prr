...
**Path resolution (review comments):** Extension fallbacks (**`tryResolvePathWithExtensionVariants`** in **`shared/path-utils.ts`**) and fragment handling (**`isReviewPathFragment`**, **`pathDismissCategoryForNotFound`**) keep **one path → one dismissal category**; legacy fragment **`missing-file`** is normalized to **`path-unresolved`** on load. Extend rules in **`path-utils`** / solvability, not ad hoc branches. **WHY one category:** If one path is sometimes **`missing-file`** and sometimes **`path-unresolved`** (e.g. bare **`.d.ts`**), state and solvability can disagree across runs and operators see churn; central rules prevent that.
