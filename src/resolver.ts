...
Start your response with \`\`\` and end with \`\`\`.`;
      ...
      while (pushIteration < maxPushIterations) {
        ...
        if (comments.length === 0) {
          ...
          if (noCommentsResult.shouldExit) {
            if (noCommentsResult.exitReason) this.exitReason = noCommentsResult.exitReason;
            if (noCommentsResult.exitDetails) this.exitDetails = noCommentsResult.exitDetails;
            break;
          }
        }
        ...
        ResolverProc.analyzeAndReportIssues(comments, unresolvedIssues, this.stateManager, analyzeTime);
        ...
        while (fixIteration < maxFixIterations && !allFixed) {
          ...
          // Verify fixes
          const { verifiedCount, failedCount, changedIssues, unchangedIssues } = await ResolverProc.verifyFixes(
            git,
            unresolvedIssues,
            this.stateManager,
            this.lessonsManager,
  ...
  /**
   * Run cleanup-only mode: remove prr artifacts from the PR and exit.
   * 
   * WHY: Sometimes you want to clean up without running the full fix loop.
   * --clean-claude-md: Remove prr section from CLAUDE.md (or delete file if only prr content)
   * --clean-state: Remove .pr-resolver-state.json from git
  ...
  private async runCleanupMode(
    ...
    const cleanClaudeMd = this.options.cleanClaudeMd || this.options.cleanAll;
    const cleanState = this.options.cleanState || this.options.cleanAll;
    
    // Get PR info to know the branch
    spinner.start('Fetching PR information...');
    this.prInfo = await this.github.getPRInfo(owner, repo, prNumber);
    spinner.succeed(`PR branch: ${this.prInfo.branch}`);
    
    // Set up workdir
    const { path: workdirPath, exists: workdirExists } = getWorkdirInfo(this.config.workdirBase, owner, repo, prNumber, this.prInfo.branch);
    ...
    await cloneOrUpdate(this.workdir, this.prInfo.cloneUrl, this.prInfo.branch);
